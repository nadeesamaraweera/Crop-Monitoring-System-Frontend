...
function updateTableRow(crop, index) {
    ...
    row.cells[2].textContent = crop.scientificName;
    row.cells[3].textContent = crop.category;
...
tableBody.addEventListener("click", function (event) {
    ...
    if (event.target.classList.contains("edit-btn")) {
        ...
        document.getElementById("updateCropSeason").value = crop.season;
        document.getElementById("cropFieldUpdate").value = crop.fields;

        const imgSrc = row.cells[5].querySelector("img") ;
...
document.getElementById("updateCropForm").addEventListener("submit", function (event) {
    ...
    if (editingRow) {
        ...
        const cropScientificName = document.getElementById("updateCropScientificName").value.trim();
        const cropCategory = document.getElementById("updateCropCategory").value.trim();
        ...
        const cropFieldId = document.getElementById("updateFieldId;").value;
        ...
        cells[5].innerText = cropFieldId;
        if (cropImage) {
            cells[6].querySelector("img").src = URL.createObjectURL(cropImage);
        }
        alert("Crop updated successfully!");
        editingRow = null;
